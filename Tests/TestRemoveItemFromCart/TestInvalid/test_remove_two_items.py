...
class TestItemToCart:
    ...
    def make_sure_the_cart_is_not_empty(self):
        ...
        index = 1
        while itemPage.get_quantity_in_cart() < 2:
    ...
    def test_remove_item_from_cart(self):
        ...
        for index in range(0, 2):
            itemsPage.choose_item_by_name(items_in_cart[index]).remove_from_cart()
        # 2 ->
        assert itemsPage.get_quantity_in_cart() == quantity_before - 2
        # 3 ->
        itemsPage.click_on_cart_button()
        for index in range(0, 2):
            assert checkOutPage.verify_item_in_cart(items_in_cart[index]) is False
