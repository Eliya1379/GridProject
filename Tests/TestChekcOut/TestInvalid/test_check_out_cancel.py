...
class TestCheckOutValid:
    ...
    def test_check_out_invalid_user_details(self):
        checkOutPage = CheckOutPage()
        number_of_items_in_cart = checkOutPage.get_quantity_in_cart()
        checkOutPage.click_on_check_out()
        checkOutPage.set_checkout_information("eliya", "mazoz", "1234")
        checkOutPage.click_on_continue()
        checkOutPage.click_on_cancel()
        assert number_of_items_in_cart == checkOutPage.get_quantity_in_cart()
